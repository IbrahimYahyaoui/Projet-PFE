...
export default function TeamWorkload() {
  ...
  if (!data)   return <EmptyState icon="chart-bar" title="Aucune équipe" />;
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100, mx: "auto" }}>
      <PageHeader title="Charge de travail" subtitle={`${team.name} — aide à l'assignation de tickets`} icon="chart-bar" />
                ...
                <Box sx={{ display: "flex", gap: 1.5, mt: 2 }}>
                  {[
                    ...
                    <Box key={s.label} sx={{ flex: 1, textAlign: "center", py: 0.8, borderRadius: "8px", bgcolor: C.bgPage }}>
                      <Typography sx={{ fontFamily: "Inter, sans-serif", fontSize: "16px", fontWeight: 700, color: s.color }}>{s.value}</Typography>
                      <Typography sx={{ fontFamily: "Inter, sans-serif", fontSize: "10px", color: C.textMuted }}>{s.label}</Typography>
                    </Box>
                  ))}
                </Box>
              </Box>
