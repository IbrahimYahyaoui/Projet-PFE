...
const AllTickets = () => {
  ...
  const stats = [
    { label: "Total tickets", value: tickets.length, icon: <ConfirmationNumber sx={{ fontSize: 20 }} />, color: C.accent },
  ...
  return (
    <Box sx={{ flex: 1, p: 4, bgcolor: C.bg }}>
      {/* Header */}
      <Box sx={{ mb: 4 }}>
        <Typography fontSize={26} fontWeight={700} color={C.textPrimary}>All tickets</Typography>
        <Typography fontSize={14} color={C.textSecondary} mt={0.5}>Manage all company tickets</Typography>
      </Box>
      ...
      <Box sx={{ bgcolor: C.card, borderRadius: 3, border: `1px solid ${C.border}`, overflow: "hidden" }}>
        <Box sx={{ px: 3, py: 2.5, borderBottom: `1px solid ${C.border}` }}>
          <Typography fontSize={16} fontWeight={700} color={C.textPrimary}>All company tickets</Typography>
        </Box>
            ...
            <Table>
              <TableHead>
                <TableRow sx={{ bgcolor: C.tableHeader }}>
                  {["Title", "Status", "Priority", "Category", "Created by", "Assigned to", "Created at", "Actions"].map((col) => (
                    ...
                    <TableCell>
                      {ticket.assignedTo ? (
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          ...
                          <Typography fontSize={13} color={C.textSecondary}>{ticket.assignedTo.name}</Typography>
                        </Box>
                      ) : (
                        <Typography fontSize={13} fontStyle="italic" color={C.textMuted}>Unassigned</Typography>
                      )}
                    </TableCell>
                    <TableCell>
